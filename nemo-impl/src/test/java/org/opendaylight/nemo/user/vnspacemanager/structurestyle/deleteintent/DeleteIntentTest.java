...
public void styleNemoDeleteOutputTest(){
	 ...
	 when(styleNemoDeleteInput.getUserName()).thenReturn(usernametest);
	 when(styleNemoDeleteInput.getUserPassword()).thenReturn(userpasswordtest);
	 when(styleNemoDeleteInput.getUserRole()).thenReturn(userrolenametest);
	 //branch 1
	 when(aaa.checkUser(styleNemoDeleteInput)).thenReturn("The user is not exist.");
	 Assert.assertEquals(deleteintent.styleNemoDeleteOutput(aaa,styleNemoDeleteInput),"The user is not exist.");
	 //branch 2
	 when(aaa.checkUser(styleNemoDeleteInput)).thenReturn(null);
